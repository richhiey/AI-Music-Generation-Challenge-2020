...
def create_RNN_layer(cells, go_backwards = False):
...
def create_rnn_from_config(model_configs):
    if model_configs['bidirectional'] == 'True':            
        stacked_fwd_cells = tf.keras.layers.StackedRNNCells(
            create_RNN_cells(model_configs['rnn'])
        )
        stacked_bwd_cells = tf.keras.layers.StackedRNNCells(
            create_RNN_cells(model_configs['rnn'])
        )

        forward_RNN = create_RNN_layer(stacked_fwd_cells)
        backward_RNN = create_RNN_layer(stacked_bwd_cells, True)
        ...
        sequential_RNN = create_RNN_layer(stacked_cells)
        return sequential_RNN
...
class SequenceToSequence(tf.keras.Model):
    ...
    def __init__(self, model_path, data_dimensions):
        ...
        model_config_path = os.path.join(model_path, 'seq2seq.json')
    ...
    def __create_model__(self, model_configs, data_dimensions):
        ...
        #----------------------------------------
        encoder_rnn = create_rnn_from_config(self.model_configs)
        enc_output, enc_hidden_state, _ = encoder_rnn(full_input, mask = mask)
        #----------------------------------------
        # ADD SEQ2SEQ CODE HERE!!   
        attention_context_vector, attention_weights = self.attention(enc_hidden_state, enc_output)
        decoder_rnn = create_rnn_from_config(self.model_configs)
        dec_output, dec_hidden_state, _ = decoder_rnn(attention_context_vector, mask = mask)
        #----------------------------------------
        next_tokens = tf.keras.layers.Dense(data_dimensions['tune_vocab_size'])(dec_output)
        #----------------------------------------
